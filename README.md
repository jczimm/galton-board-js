...
1. **`src/extra/from-stl.js`** (`from-stl.html`) — the one under active development. It
   loads a printable board straight from an STL and drops 0.5mm steel balls through it with
   [Rapier](https://rapier.rs/) (`@dimforge/rapier3d-simd`), so the thing being simulated is
   the same mesh that gets printed. This is what the analysis below measures.
2. **`src/galton-board.js` + `src/component.js`** — the original LIT web component, with
   pegs placed procedurally and cannon-es physics. Documented in the rest of this file.

## The STL board

```bash
pnpm dev            # then open /from-stl.html
```

Everything is a URL parameter: `?model=boarddef&seed=1&balls=800&ballRest=.85&tilt=0`.
Boards come from `cad/models/` (`board_def`, `recreate_original_board`, `clear_board`),
generated by the OpenSCAD sources beside them. A run ends by itself once every ball has
settled, and exports one CSV of final ball positions whose filename encodes every parameter
plus a `phys-N` physics version — runs from different geometry versions are not comparable,
so the tag keeps them apart.

Sweeps are automated with Playwright; each param takes a comma-separated list and the grid
is their cartesian product:

```bash
pnpm sweep --balls 800 --seed 1,2,3 --ballRest .5,.7,.85
```

CSVs land in `analysis/data/`, where a small Python project (`uv run python analysis/main.py`)
buckets them against bucket edges derived from the STL itself, compares the result to any
target distribution, and — in `analysis/photo.py` — reads the same distribution off
photographs of the real printed board. See `TODO.md` for what's been established so far.
...
└── extra/from-stl.js  # STL board sim (Rapier) -- see top of this file
cad/                   # OpenSCAD sources and the STLs they produce
scripts/sweep.mjs      # Playwright sweep driver
analysis/              # Python: bucket the CSVs, compare distributions, read the photos
```
