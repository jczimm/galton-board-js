...
- [ ] prevent balls from entering the holes on the bottom edge of the model
- [ ] try adding back some randomness to the positions of the balls in initialization so that we can get a sampling distribution of the mean of the output curve and ensure that its mean approaches 0 (otherwise, we still have some unexpected asymmetry!)
